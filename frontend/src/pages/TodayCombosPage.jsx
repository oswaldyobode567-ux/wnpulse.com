...
import { Shield, ShieldCheck, Zap, Flame, Rocket, Loader2, Lock, Send, Trophy, Sparkles, AlertTriangle } from "lucide-react";
...
const TIER_META = {
ultra_safe: { icon: ShieldCheck, accent: "teal", gradient: "from-teal-500 to-cyan-700" },
...
Uniquement les matchs qui se jouent <strong>aujourd'hui</strong>, répartis en niveaux de cote — du plus sûr au plus jackpot. Choisis ta stratégie.
</p>
</div>

    {/\* Ultra-Safe — tous sports confondus, toujours visible en tete \*/}
    {!loading && data?.ultra\_safe && (
      \<div className="mb-6">
        \<TierCard
          tier={data.ultra\_safe}
          tkey="ultra\_safe"
          isFree={false}
          onShare={() => shareWhatsApp(data.ultra\_safe)}
        />
      \</div>
    )}
    ...
    \<Tabs value={sportKey} onValueChange={setSportKey} className="mb-6">
      \<TabsList className="bg-white border border-neutral-200 flex-wrap h-auto p-1" data-testid="today-sport-tabs">
        {families.map((f) => (
          \<TabsTrigger key={f.family\_key} value={f.family\_key} disabled={f.matches\_today === 0 && f.family\_key !== "all"} data-testid={\`today-tab-${f.family\_key}\`}>
            {f.family\_label}
            {f.matches\_today > 0 && \<span className="ml-1.5 text-[9px] opacity-70">{f.matches\_today}\</span>}
          \</TabsTrigger>
        ))}
      \</TabsList>
    \</Tabs>
    {loading ? (
      ...
      \</div>
    ) : !currentFamily || currentFamily.matches\_today === 0 ? (
      \<Card className="p-8 bg-white border-neutral-200 text-center">
        \<div className="text-4xl mb-3 opacity-40">🕒\</div>
        \<p className="text-slate-500 text-sm">Aucun match aujourd'hui dans \<strong>{currentFamily?.family\_label || "ce sport"}\</strong>.\</p>
...
const isUltraSafe = tkey === "ultra_safe";
const potentialWin = Math.round((tier.total_odds || 0) * 1000);
return (
<Card className={cn("bg-white overflow-hidden", isUltraSafe ? "border-teal-300 ring-2 ring-teal-500/10" : "border-neutral-200")} data-testid={`today-tier-${tkey}`}>
...
Même un favori très net peut perdre. Ce niveau réduit le risque, il ne l'élimine pas. Jamais de garantie à 100% sur un pari sportif.
</p>
</div>
)}
{isLocked ? (
...
</div> <span className="text-[10px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded">{p.confidence?.toFixed(0)}%</span>
</div>
